...
/ .v0.6 - played with some components to get load cell to read continously Lines(158-166,560-568)
/
////
/
...
void loop() {
  ...
  weight = readLoadCellWeight();  // reads the current weight value from the load cell during EACH loop cycle
...
float readLoadCellWeight() {
  //local vars
  float calculatedLoad = 0;                // stores the values read from the load cell for processing
...
void drawScreen(int menuPageShown, int menuItemSelected) {
  ...
  if (menuPageShown == 3) {
    ...
    tft.setCursor(9, 0);
    tft.print("HELP");
    // ***TYLER*** - You'll have to insert what you want to show for the HELP screen here    
  }
